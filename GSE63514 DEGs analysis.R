...
Rawdata<- rma(Rawdata, normalize = T, background = T)
Expression_probe <- exprs(Rawdata)
Table <- data.frame(row.names = filename_use,group = c(rep('normal',24),rep("cancer",28)))
...
#Volcano plot
topDEG <- c(head( Result_all[order(desc(Result_all$logFC)),], n = 30)$gene_name,
            tail( Result_all[order(desc(Result_all$logFC)),], n = 30)$gene_name)
Result_all$topDEG <- ifelse(Result_all$gene_name %in% topDEG,Result_all$gene_name, NA)

volcano_plot1 <- Result_all %>%
  ...
  labs(title= 'Differentially expressed genes of Cancer VS Normal- 63514',tag='Volcano plot',
