...
estimateScore(gene_expression7803,"EstimateScore7803.gct", platform = c("affymetrix", "agilent", "illumina"))
Estimatescore7803 <- system.file("extdata", "EstimateScore7803.gct", package="estimate")
...
# Filter Common genes
CervicalCancerExpr <- system.file("extdata", "7803_raw.txt", package="estimate")
filterCommonGenes(input.f= CervicalCancerExpr, output.f="common_genes.gct", id=c("GeneSymbol","EntrezID"))
...
stopifnot((is.character("common_genes.gct") && length("common_genes.gct") == 1 && nzchar("common_genes.gct")))
id <- match.arg(id)
...
#Part3
?estimateScore
estimateScore("gene_expression7803.gct", "EstimateScore7803.gct", platform = "affymetrix")
## Check arguments
stopifnot(is.character("gene_expression7803.gct") && length("gene_expression7803.gct") == 1 && nzchar("gene_expression.gct"))
stopifnot(is.character("SI_geneset.gct") && length("SI_geneset.gct") == 1 && nzchar("SI_geneset.gct"))
...
ds <- read.delim("gene_expression.gct", header=TRUE, sep="\t", skip=2, row.names=1, blank.lines.skip=TRUE, as.is=TRUE, na.strings = "")
descs <- ds[,1]
ds <- ds[-1]
...
temp <- strsplit("gene_expression.gct", split="/")
s <- length(temp[[1]])
...
if (length(gene.overlap) == 0)
  score.matrix[gs.i, ] <- rep(NA, Ns)else
...
### ข้อมูลเป็น Affymatrix ทั้งคู่ (platform != "affymetrix")
score.data <- rbind(score.data, estimate.score)
rownames(score.data) <- c("StromalScore", "ImmuneScore", "ESTIMATEScore") else

  ## Calculate ESTIMATE-based tumor purity (This function is limited in the Affymetrix platform)
  convert_row_estimate_score_to_tumor_purity <- function( estimate.score)
    stopifnot(is.numeric( estimate.score))
cos(0.6049872018 + 0.0001467884* estimate.score)
...
outputGCT(score.data, output.f  ="EstimateScore63514.gct")
invisible(NULL)


#Part4: Plot
plotPurity("EstimateScore7803.gct",
           samples="all_samples",
           platform = "affymetrix",
           output.dir="estimated_purity_plots") {

  ## Check arguments
  stopifnot((is.character("EstimateScore63514.gct") && length("EstimateScore63514.gct") == 1 && nzchar("EstimateScore63514.gct")) ||
              (inherits("EstimateScore63514.gct", "connection") && isOpen("EstimateScore63514.gct", "r")))
  stopifnot(is.character("PurityDataAffy.gct") && length("PurityDataAffy.gct") == 1 && nzchar("PurityDataAffy.gct"))
  platform <- match.arg(platform)
...
##---------------------------------------------------------------------------
get_estimates_df <- function(EstimateScore63514.gct) {
  #estimate <- read.table("EstimateScore63514.gct", skip=2, header=TRUE, row.names=1, sep="\t")
  estimate <- read.delim(scores, skip=2, row.names=1)
  as.data.frame(t(estimate[, -1]))
}

##---------------------------------------------------------------------------
convert_row_estimate_score_to_tumor_purity <- function(get_estimates_df) {
  stopifnot(is.numeric(get_estimates_df))
  cos(0.6049872018 + 0.0001467884*get_estimates_df)
}
...
estimate.df <- get_estimates_df(EstimateScore63514.gct)
